...
class PooledInstanceProxy {
  ...
  Future<R> callRemoteMethod<R>(Action action) {
    if (_pool.state == IsolatePoolState.stoped) {
      throw IsolatePoolStopped(
          'Isolate pool has been stoped, cant call pooled instnace method');
    }
...
class IsolatePool {
  ...
  Future<T> scheduleJob<T>(PooledJob job) {
    if (state == IsolatePoolState.stoped) {
      throw IsolatePoolStopped(
          'Isolate pool has been stoped, cant schedule a job');
    }
  ...
  void destroyInstance(PooledInstanceProxy instance) {
    ...
    if (index == -1) {
      throw NoSuchIsolateInstance(
          'Cant find instance with id ${instance._instanceId} among active to destroy it');
    }
  ...
  Future<R> _sendRequest<R>(int instanceId, Action action) {
    if (!_pooledInstances.containsKey(instanceId)) {
      throw NoSuchIsolateInstance(
          'Cant send request to non-existing instance, instanceId $instanceId');
    }
    var pim = _pooledInstances[instanceId]!;
    if (pim.state == _PooledInstanceStatus.starting) {
      throw IsolateNotYetStarted(
          'Cant send request to instance in Starting state, instanceId $instanceId}');
    }
  ...
  void stop() {
    for (var i in _isolates) {
      ...
      for (var c in jobCompleters.values) {
        if (!c.isCompleted) {
          c.completeError(IsolatePoolJobCancelled(
              'Isolate pool stopped upon request, cancelling jobs'));
        }
      }
      jobCompleters.clear();
...
void _processResponse(_Response response,
  ...
  if (!cc.containsKey(response.requestId)) {
    throw BadResponseReceivedError(
        'Responnse to non-existing request (id ${response.requestId}) recevied');
  }
